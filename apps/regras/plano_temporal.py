...
from apps.regras.rules.plano_temporal import (
    avaliar_execucao_temporal_item,
)


@dataclass
class ResultadoExecucaoTemporalItem:
    item_id: int
    resumo: object
...
def analisar_execucao_temporal_item(
    ...
    achados = avaliar_execucao_temporal_item(
        resumo
    )

    return ResultadoExecucaoTemporalItem(
        item_id=item.pk,
        resumo=resumo,
        achados=achados,
    )
